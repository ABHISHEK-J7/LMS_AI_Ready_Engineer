    ...
    <>
      ...
      {isLoading && !items ? (
        ...
        <EmptyState
          ...
          description="No assessments are available yet. Your trainer unlocks practice tests after completing each syllabus section."
        />
      ) : (
        <div className="module-grid">
          {items?.map((a) => {
            ...
            const windowClosed = a.proctored && a.deadline && new Date(a.deadline).getTime() < Date.now();
            return (
              <Card key={a.id} className="module-card">
                ...
                <div className="module-card__meta">
                  ...
                  {windowClosed && !done && <Badge tone="neutral">Closed</Badge>}
                </div>
                {a.gated && (
                ...
                <div className="list-actions">
                  {done ? (
                    ...
                    <Button size="sm" variant="outline" disabled>
                      {windowClosed ? 'Closed' : a.gated ? 'Locked' : 'Not available'}
                    </Button>
                  )}
                </div>
              </Card>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)', padding: 'var(--space-3)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)' }}>
      ...
      {form.proctoring === ProctoringMode.SEB && (
        <span className="lms-muted" style={{ fontSize: 'var(--font-size-xs)' }}>
          Set the global SEB Config Key in admin Settings, and make sure students launch via the .seb config.
        </span>
      )}
      <span className="lms-muted" style={{ fontSize: 'var(--font-size-xs)' }}>
...
function StaffAssessments() {
  ...
  async function submitCreate(e) {
    ...
    try {
      ...
      // Proctored modes (app/seb) carry the exam window + duration.
    ...
    <>
      ...
      {!moduleId ? (
        ...
        <EmptyState
          ...
          description="No assessments yet for this module. Create the first practice test."
